...
router.delete('/:id', validateProjectID, (request, response, next) => {
    Projects.remove(request.params.id)
        .then(() => {
...
router.get('/:id/actions', validateProjectID, (request, response, next) => {
    Projects.getProjectActions(request.params.id)
        .then(actions => {
